...
impl Cipher {
    ...
    pub fn new(raw: Option<&[u8]>, code: Option<&str>) -> Result<Self, MatterError> {
        ...
        let code = match (raw, code) {
            (Some(r), None) => {
                if r.len() == raw_size(mtr_dex::X25519_CIPHER_SALT)? {
                    // X25519_Cipher_Salt size
                    mtr_dex::X25519_CIPHER_SALT
                } else if r.len() == raw_size(mtr_dex::X25519_CIPHER_SEED)? {
                    // X25519_Cipher_Seed size
            ...
            _ => {
                return Err(MatterError::EmptyMaterial(
                    "Both raw and code cannot be None for Cipher".to_string(),
                ))
            }
        };
    ...
    fn is_supported_code(code: &str) -> bool {
        ...
        matches!(
            code,
            mtr_dex::X25519_CIPHER_SEED | mtr_dex::X25519_CIPHER_SALT
        ) // X25519_Cipher_Seed, X25519_Cipher_Salt
    }
}
...
mod tests {
    ...
    fn test_cipher() -> Result<(), MatterError> {
        ...
        assert_eq!(
            std::str::from_utf8(&seed_qb64b).unwrap(),
        ...
        assert_eq!(
            std::str::from_utf8(&salt_qb64b).unwrap(),
        ...
        let cipher = Cipher::new(Some(&raw), Some(mtr_dex::X25519_CIPHER_SEED))
        ...
        // Test .decrypt method
        let pri_key_matter = BaseMatter::new(
            Some(pri_key.as_ref()),
            Some(mtr_dex::X25519_PRIVATE),
            None,
            None,
        )
        ...
        let decrypted = cipher
            .decrypt(Some(&pri_key_qb64b), None, None, Some(false))
        ...
        let decrypted_bare = cipher
            .decrypt(Some(&pri_key_qb64b), None, None, Some(true))
            .expect("Failed to decrypt with prikey (bare)");
        let bare_bytes = decrypted_bare
            .downcast_ref::<Vec<u8>>()
            .expect("Failed to downcast to Vec<u8>");
        assert_eq!(bare_bytes, &seed_qb64b);

        // Test decryption with seed
        let crypt_seed_matter =
            BaseMatter::new(Some(&crypt_seed), Some(mtr_dex::ED25519_SEED), None, None)
                .expect("Failed to create crypt seed matter");
        let crypt_seed_qb64b = crypt_seed_matter.qb64b();

        let decrypted_from_seed = cipher
            .decrypt(None, Some(&crypt_seed_qb64b), None, Some(false))
            .expect("Failed to decrypt with seed");
        let decrypted_seed_matter = decrypted_from_seed
            .downcast_ref::<Signer>()
            .expect("Failed to downcast to Matter");
        assert_eq!(decrypted_seed_matter.qb64b(), seed_qb64b);

        // Test bare decryption with seed
        let decrypted_seed_bare = cipher
            .decrypt(None, Some(&crypt_seed_qb64b), None, Some(true))
            .expect("Failed to decrypt with seed (bare)");
        let seed_bare_bytes = decrypted_seed_bare
            .downcast_ref::<Vec<u8>>()
            .expect("Failed to downcast to Vec<u8>");
        assert_eq!(seed_bare_bytes, &seed_qb64b);

        // Test wrong but shorter code
        let wrong_code_cipher = Cipher::new(Some(&raw), Some(mtr_dex::X25519_CIPHER_SALT))
            .expect("Failed to create cipher with wrong code");
        ...
        // Test Cipher creation with explicit code
        let salt_cipher = Cipher::new(Some(&salt_raw), Some(mtr_dex::X25519_CIPHER_SALT))
            .expect("Failed to create salt cipher with code");
        assert_eq!(salt_cipher.code(), mtr_dex::X25519_CIPHER_SALT); // X25519_Cipher_Salt

        let salt_uncb =
            open(&salt_cipher.raw(), &pub_key, &pri_key).expect("Failed to decrypt salt");
        assert_eq!(salt_uncb, salt_qb64b);
