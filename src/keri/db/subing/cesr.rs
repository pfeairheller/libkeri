...
mod tests {
    ...
    fn test_cesr_suber() -> Result<(), CesrSuberError> {
        ...
        // Test get operation
        let actual = sdb.get(keys).unwrap().unwrap();
        assert_eq!(actual.qb64(), val0.qb64());

        // // Test trim (remove) operation
        // let result = sdb.trim(keys, false).unwrap();
        // assert!(result);
        //
        ...
        // let trim_result = sdb.trim(keys, false).unwrap();
        // assert!(trim_result);
        //
        // let put_result = sdb.put(keys, &val1).unwrap();
        // assert!(put_result);
        //
        // let actual = sdb.get(keys).unwrap().unwrap();
        // assert_eq!(actual.qb64().unwrap(), val1.qb64().unwrap());
        //
        // // Test with keys as string
        // let keys_str = &[b"beta.fish".as_ref()];
        //
        // let result = sdb.put(keys_str, &val1).unwrap();
        // assert!(result);
        //
        // let actual = sdb.get(keys_str).unwrap().unwrap();
        // assert_eq!(actual.qb64().unwrap(), val1.qb64().unwrap());
        //
        // let result = sdb.trim(keys_str, false).unwrap();
        // assert!(result);
        //
        ...
        //
        // // Test missing entry
        // let bad_key = &[b"badkey".as_ref()];
        // let actual = sdb.get(bad_key).unwrap();
        // assert!(actual.is_none());
        //
        // // Test iteritems (get_item_iter)
        // let sdb_pugs = CesrSuber::<Matter>::new(db_ref.clone(), "pugs.", None, true).unwrap();
        //
        // let keys_a1 = &[b"a".as_ref(), b"1".as_ref()];
        // let keys_a2 = &[b"a".as_ref(), b"2".as_ref()];
        //
        // assert!(sdb_pugs.put(keys_a1, &val0).unwrap());
        // assert!(sdb_pugs.put(keys_a2, &val1).unwrap());
        //
        // let items = sdb_pugs.get_item_iter(&[], false).unwrap();
        // let processed_items = sdb_pugs.process_items(items).unwrap();
        //
        ...
        //         .collect();
        //
        //     result_items.push((keys_tuple, val.qb64().unwrap()));
        // }
        //
        // // Check the iteritems result
        // assert_eq!(result_items.len(), 2);
        // assert_eq!(result_items[0].0, vec!["a".to_string(), "1".to_string()]);
        // assert_eq!(result_items[0].1, val0.qb64().unwrap());
        // assert_eq!(result_items[1].0, vec!["a".to_string(), "2".to_string()]);
        // assert_eq!(result_items[1].1, val1.qb64().unwrap());
        //
        // // Test with Diger class
        // let sdb_diger = CesrSuber::<Diger>::new(db_ref.clone(), "pigs.", None, true).unwrap();
        //
        // let dig0 = "EAPYGGwTmuupWzwEHHzq7K0gzUhPx5_yZ-Wk1x4ejhcc";
        // let val0 = Diger::new_with_qb64(dig0).unwrap();
        //
        // let keys = &[b"alpha".as_ref(), b"dog".as_ref()];
        //
        // assert!(sdb_diger.put(keys, &val0).unwrap());
        //
        // let actual = sdb_diger.get(keys).unwrap().unwrap();
        // assert_eq!(actual.qb64().unwrap(), val0.qb64().unwrap());
        //
        // assert!(sdb_diger.trim(keys, false).unwrap());
        //
        ...
        // let val1 = Matter::new_with_qb64(pre1).unwrap();
        //
        // // This should fail because we're trying to add a different type
        // let result = sdb_diger.put(keys, &val1).unwrap();
        // assert!(!result);
        //
        // // Force update with pin operation (trim and put)
        // let trim_result = sdb_diger.trim(keys, false).unwrap();
        // assert!(trim_result);
        //
        // let val1_diger = Diger::new_with_qb64(pre1).unwrap();
        // let put_result = sdb_diger.put(keys, &val1_diger).unwrap();
        // assert!(put_result);
        //
        // let actual = sdb_diger.get(keys).unwrap().unwrap();
        // assert_eq!(actual.qb64().unwrap(), val1_diger.qb64().unwrap());
        //
        // // Test iteritems with another database
        // let sdb_figs = CesrSuber::<Diger>::new(db_ref.clone(), "figs.", None, true).unwrap();
        //
        // let keys_a1 = &[b"a".as_ref(), b"1".as_ref()];
        // let keys_a2 = &[b"a".as_ref(), b"2".as_ref()];
        //
        // assert!(sdb_figs.put(keys_a1, &val0).unwrap());
        // assert!(sdb_figs.put(keys_a2, &val1_diger).unwrap());
        //
        // let items = sdb_figs.get_item_iter(&[], false).unwrap();
        // let processed_items = sdb_figs.process_items(items).unwrap();
        //
        // let mut result_items = Vec::new();
        // for (keys_vec, val) in processed_items {
        //     let keys_tuple: Vec<String> = keys_vec.into_iter()
        ...
        // assert_eq!(result_items[0].1, val0.qb64().unwrap());
        // assert_eq!(result_items[1].0, vec!["a".to_string(), "2".to_string()]);
        // assert_eq!(result_items[1].1, val1_diger.qb64().unwrap());
        //
        // // Add more data
        // let keys_b1 = &[b"b".as_ref(), b"1".as_ref()];
        // let keys_b2 = &[b"b".as_ref(), b"2".as_ref()];
        // let keys_bc1 = &[b"bc".as_ref(), b"1".as_ref()];
        //
        // assert!(sdb_figs.put(keys_b1, &val0).unwrap());
        // assert!(sdb_figs.put(keys_b2, &val1_diger).unwrap());
        // assert!(sdb_figs.put(keys_bc1, &val0).unwrap());
        //
        // // Test with topkey (prefix filtering)
        // let top_keys = &[b"b".as_ref(), b"".as_ref()];
        // let items = sdb_figs.get_item_iter(top_keys, true).unwrap();
        // let processed_items = sdb_figs.process_items(items).unwrap();
        //
        // let mut result_items = Vec::new();
        // for (keys_vec, val) in processed_items {
        ...
        // let val0 = Siger::new_with_qb64(sig0).unwrap();
        //
        // let keys = &[b"zeta".as_ref(), b"cat".as_ref()];
        //
        // assert!(sdb_siger.put(keys, &val0).unwrap());
        //
        // let actual = sdb_siger.get(keys).unwrap().unwrap();
        // assert_eq!(actual.qb64().unwrap(), val0.qb64().unwrap());
